...
SUMMARY = Path(__file__).parent.parent / "results/summary"
...
MARKETS = {
    ...
    "Nordic Markets": {
        "exchanges": ["Stockholm", "Oslo", "Copenhagen"],
...
def EOD(scale={"volume": 1, "price": 1}):
    min_volume = f"volume>{250000*scale['volume']}"
    min_average_volume = f"sma(50,volume)>{250000*scale['volume']}"
    min_dollar_volume = f"volume*close>{2500000*scale['volume']*scale['price']}"
    min_average_dollar_volume = (
        f"sma(50,volume)*sma(50)>{2500000*scale['volume']*scale['price']}"
    )
    ...
    eod = {
        ...
        "Mark Minvervini": {
            ...
            "scans": {
                "Trend Template": {
                    "query": "close>sma(150) AND close>sma(200) AND sma(150)>sma(200) AND sma(200,close(22))<sma(200) AND sma(50)>sma(150) AND sma(50)>sma(200) AND close/min(252)>=1.25 AND close/max(252)>=0.75 AND ibd_rs_rank>70",
                    "order": "ibd_rs",
                    "limit": 200,
                }
            },
